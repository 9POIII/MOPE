...
def func(x1, x2, x3):
    coefficients = [0.6, 8, 8.8, 9.2, 4.7, 0.1, 1.1, 6.3, 0.4, 1.3, 2.9]
...
def cochranCriteria(m, n, y_table):
    ...
    time_cochran = time.perf_counter()
    print("Час перевірки однорідності дисперсії за Кохреном: " + str(time_cochran))
...
def studentCriteria(m, n, y_table, betaCoeffs):
    ...
    time_student = time.perf_counter()
    print("Час перевірки однорідності дисперсії за Стьюдентом: " + str(time_student))
...
def fisherCriteria(m, N, d, tableX, tableY, coeffsB, importance):
    ...
    time_fisher = time.perf_counter()
    print("Час перевірки однорідності дисперсії за Фішером: " + str(time_fisher))
